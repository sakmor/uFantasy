...
public class CharacterSystem : IGameSystem
{
    ...
    public CharacterSystem(mainGame mainGame) : base(mainGame)
    {
        Initialize();

        // 註冊事件
        // fixme:不確定這要註冊什麼事件
        //  mainGame.RegisterGameEvent(ENUM_GameEvent.NewStage, new NewStageObserverSoldierAddMedal(mainGame));
    }
    ...
    public void RemoveCharacter(List<ICharacter> Characters, List<ICharacter> Opponents, ENUM_GameEvent emEvent)
    {
        ...
        foreach (ICharacter Character in Characters)
        {
            ...
            if (Character.CheckKilledEvent() == false)
                mainGame.NotifyGameEvent(emEvent, Character);
        ...
        {
            // 通知對手移除
            // foreach (ICharacter Opponent in Opponents) Opponent.RemoveAITarget(CanRemove);
    ...
    */
    ...
    {
        // foreach (ICharacter Character in Characters)            Character.UpdateAI(Targets);
    }
